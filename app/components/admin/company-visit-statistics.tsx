...
export const CompanyVisitStatistics = () => {
    ...
    const handleChange = (event: React.ChangeEvent<unknown>, value: number) => {
        ...
        HttpService.httpService(`company?page=${value}&limit=10`, 'GET').then((res) => {
            setCurrentState(res?.data?.items ?? [])
            setMetadata(res?.data?.meta)
            setPage(value);
        })
            ...
            <main>
                ...
                <div className="difContainer difBloc">
                    <div className="difHeadings difFlex">
                        <h2>Statistics des Contacts ({currentState?.length.toString()}) </h2>
                    </div>

                    {!currentState?.length ? (<span>Auccun Entreprise</span>) : (
                    ...
                    <nav className="difPagination">
                        <Pagination count={metaData?.totalPages} page={metaData?.currentPage ?? 1}
